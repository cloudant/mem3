...
-module(mem3_replication_manager).
-behaviour(gen_fsm).
...
-export([idle/2, idle/3, active/2, active/3]).
...
init(_) ->
    ...
    DbName = ?l2b(couch_config:get("mem3", "replicator_db", "_replicator")),
    try fabric:get_db_info(DbName),
         twig:log(info, "Monitoring existing replicator db.", []),
...
start_replicator() ->
    ...
    ok = couch_config:register(
        fun("mem3", "replicator_db", NewName) ->
            ok = gen_fsm:send_event(Server, {rep_db_changed, ?l2b(NewName)});
        ("mem3", "max_replication_retry_count", V) ->
            ok = gen_fsm:send_event(Server, {set_max_retries, retries_value(V)})
        end
    ),
...
await_creation() ->
    ...
    #idle_state{changes_pid = Pid}.

idle({rep_db_created, _DbName}, _State) ->
    twig:log(notice, "Monitoring newly created replicator db.", []),
    {next_state, active, start_replicator()};
idle(_Event, State) ->
...
active({rep_started, RepId}, _From, State) ->
    case rep_state(RepId) of
    nil ->
        ok;
    RepState ->
        NewRepState = RepState#rep_state{
            starting = false,
            retries_left = State#active_state.max_retries,
            max_retries = State#active_state.max_retries,
            wait = ?INITIAL_WAIT
        },
...
active({rep_complete, RepId}, _From, State) ->
    true = ets:delete(?REP_TO_STATE, RepId),
    {reply, ok, active, State};

active({rep_error, RepId, Error}, _From, State) ->
    {reply, ok, active, replication_error(State, RepId, Error)};

active(Msg, From, State) ->
    ?LOG_ERROR("Replication manager received unexpected call ~p from ~p",
        [Msg, From]),
    {stop, {error, {unexpected_call, Msg}}, State}.


active({rep_db_changed, NewName}, #active_state{rep_db_name = NewName} = State) ->
    {next_state, active, State};

active({rep_db_changed, _NewName}, State) ->
    {next_state, active, restart(State)};

active({rep_db_created, NewName}, #active_state{rep_db_name = NewName} = State) ->
    {next_state, active, State};

active({rep_db_created, _NewName}, State) ->
    {next_state, active, restart(State)};

active({set_max_retries, MaxRetries}, State) ->
    {next_state, active, State#active_state{max_retries = MaxRetries}};
...
handle_info({'DOWN', _, _, Pid, Reason}, idle, #idle_state{changes_pid=Pid} = State) ->
    twig:log(notice, "~p changes listener died ~p", [?MODULE, Reason]),
    Seq = case Reason of {seq, EndSeq} -> EndSeq; _ -> 0 end,
    erlang:send_after(5000, self(), {start_listener, Seq}),
    {next_state, idle, State};
handle_info({start_listener, Seq}, idle, State) ->
    {NewPid, _} = spawn_monitor(fun() -> listen_for_changes(Seq) end),
    {next_state, idle, State#idle_state{changes_pid=NewPid}};

handle_info({'EXIT', From, normal}, active, #active_state{changes_feed_loop = From}) ->
    % replicator DB deleted
    twig:log(notice, "Unmonitoring deleted replication db.", []),
    {next_state, idle, await_creation()};

handle_info({'EXIT', From, Reason}, active, #active_state{db_notifier = From} = State) ->
    ?LOG_ERROR("Database update notifier died. Reason: ~p", [Reason]),
    {stop, {db_update_notifier_died, Reason}, State};

handle_info({'EXIT', From, normal}, active, #active_state{rep_start_pids = Pids} = State) ->
    % one of the replication start processes terminated successfully
    {next_state, active, State#active_state{rep_start_pids = Pids -- [From]}};

handle_info({'DOWN', _Ref, _, _, _}, active, State) ->
    % From a db monitor created by a replication process. Ignore.
    {next_state, active, State};

handle_info(Msg, active, State) ->
    ?LOG_ERROR("Replication manager received unexpected message ~p", [Msg]),
    {stop, {unexpected_msg, Msg}, State}.

terminate(_Reason, idle, #idle_state{changes_pid=Pid}) ->
    exit(Pid, kill),
    ok;
...
changes_feed_loop() ->
    ...
    Pid = spawn_link(
        fun() ->
            ...
            ({change, Change}, Acc) ->
                case has_valid_rep_id(Change) andalso is_current_owner(Change) of
                true ->
...
db_update_notifier() ->
    ...
    {ok, Notifier} = couch_db_update_notifier:start_link(
        fun({created, DbName}) ->
            ...
            DbName ->
                ok = gen_fsm:send_event(Server, {rep_db_created, DbName});
            _ ->
                ok
            end;
        (_) ->
...
restart(#active_state{changes_feed_loop = Loop, rep_start_pids = StartPids} = State) ->
    stop_all_replications(),
    lists:foreach(
        fun(Pid) ->
            catch unlink(Pid),
...
maybe_start_replication(State, DocId, RepDoc) ->
    ...
    nil ->
        ...
        State#active_state{rep_start_pids = [Pid | State#active_state.rep_start_pids]};
    #rep_state{doc_id = DocId} ->
        State;
...
start_replication(Server, RepDoc, RepId, UserCtx, Wait) ->
    ...
    Pid when is_pid(Pid) ->
        ok = gen_fsm:sync_send_event(Server, {rep_started, RepId}, infinity),
...
changes_callback({change, {Change}, _}, _) ->
    RepDbName = ?l2b(couch_config:get("mem3", "replicator_db", "_replicator")),
    DbName = couch_util:get_value(<<"id">>, Change),
    case DbName of
        RepDbName ->
            gen_fsm:send_event(?MODULE, {rep_db_created, DbName});
        _Else ->
            ok
    end,
...
changes_callback(timeout, _) ->
    {ok, nil};
changes_callback({error, Reason}, _) ->
